...
pub async fn run() -> Result<()> {
    ...
    println!("2. Add your bot to the group and make it admin (with \"Manage Topics\" permission)");
    println!("3. Go to group settings → Topics → Enable");
    ...
    let thread_id = telegram::create_forum_topic(&bot_token, &group_id.to_string(), "pygmy-test")
        ...
        .context(
            "Could not create topic. Make sure:\n\
             1. The bot is an admin in the group with \"Manage Topics\" permission\n\
             2. Topics are enabled in group settings",
        )?;
